...
| Geographic scope | Two allowed locations: Sweden Central (primary) and North Europe (failover region; fixed `varAllowedLocations` list in `main.bicep`, independent of the deployment region) | Single region (swedencentral only); EU-wide (all European regions) | Two regions teach geo-redundancy; single region keeps the lab simple. Restricting to EU demonstrates data residency governance (relevant for GDPR labs in later modules). |
| Module scoping | Tags: dev RG via the local `tags.bicep` fallback; prod/platform tags + locks via the AVM resource-group module (RG location read from `existing` lookups because it is immutable); policies assigned once at subscription scope via AVM | Single module applying tags/locks/policies to all RGs; centralized policy assignment in separate stack | Per-RG modularity teaches that tags and locks are RG-scoped, while policies are subscription-scoped. Separate calls make the targeting explicit. |
...
| Tag taxonomy scope | Exactly the four canonical tags (Project, Environment, CostCenter, Owner); the former `Criticality` tag was dropped | Extended taxonomies (criticality, data classification, app id) enforced by policy | A fixed canonical set keeps what-if idempotent and policies simple; production taxonomies grow with governance maturity. |

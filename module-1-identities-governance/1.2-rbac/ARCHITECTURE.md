...
| Idempotency via `guid()` | Handled inside the AVM role-assignment modules (rg-scope/sub-scope), which name each assignment with a deterministic `guid()` of scope, role and principal | Sequential naming (role-assignment-1, role-assignment-2); random GUIDs | The AVM module encapsulates the `guid()` idempotency pattern the lab previously implemented by hand — the same assignment request re-applies idempotently. Note: `scripts/New-LabRoleAssignment.ps1` creates the same assignments through Az cmdlets under different names; the two paths are alternatives — deploying the template on top of script-created assignments fails with `RoleAssignmentExists`, which is why `Deploy-Bicep.ps1 -IncludeRoleAssignments` skips the template when all five assignments already exist. |
| Module versioning | AVM modules pinned to exact versions (resource-group `0.4.4`, role-assignment rg-scope/sub-scope `0.1.1`) per `docs/bicep-standards.md` §4.4 | Floating/latest tags; hand-written `Microsoft.Authorization/roleAssignments` resources with a managed API version | Version pinning replaces per-resource API-version management; the AVM module pins its own stable API version internally and upgrades are explicit, reviewable diffs. |

## 2. Lab simplifications vs production

| Aspect | What the lab does | What production would require | Why it matters |
|---|---|---|---|
| Principal IDs as parameters | Bicep expects pre-computed principal IDs (parAdminPrincipalId, parDeveloperGroupPrincipalId, etc.); `scripts/Deploy-Bicep.ps1 -IncludeRoleAssignments` resolves them from Entra ID (`Get-AzADUser` / `Get-AzADGroup`) and passes them as overrides, which is why `role-assignments.bicep` has no `.bicepparam` file (only `resource-groups.bicep` does) | Lookup of users/groups by name within Bicep; integration with Entra ID API; service principals registered in the tenant | Manual principal ID collection is error-prone; production tools (Terraform, AzureRM PowerShell) resolve principal names automatically. |

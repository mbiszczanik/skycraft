...
| **AVM consumption** | `load-balancer`, `dns-zone` (records via `a` / `cname`) and `private-dns-zone` (`virtualNetworkLinks`, `a`), called directly from `main.bicep`; the public-IP address lookup stays a local `existing` module, the LB frontends reference the PIPs through `existing` lookups scoped to the spoke resource groups | Hand-written resources in `load-balancer.bicep`, `dns-public.bicep`, `dns-private.bicep` (pre-v2) | docs/bicep-standards.md §4; `modules/get-public-ip.bicep` is the documented trivial-lookup fallback (§4.3) because the IPs are owned by Lab 2.1. |

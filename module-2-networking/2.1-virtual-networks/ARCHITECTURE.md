...
| **AVM consumption** | `avm/res/network/virtual-network` (VNets + subnets) and `avm/res/network/public-ip-address`, called directly from `main.bicep`; both peering directions come from the hub module's `peerings` parameter with `remotePeeringEnabled` | Hand-written `Microsoft.Network/*` resources in local modules (pre-v2); one peering resource per direction | Consuming Azure Verified Modules is what production teams do (docs/bicep-standards.md §4). Declaring all four peering links from the hub keeps the topology readable in one place, and referencing the spoke outputs orders the deployment without any `dependsOn`. |
| **Public IP zones** | Zone-redundant (AVM default `availabilityZones: [1, 2, 3]`); the Bastion public IP created in Lab 2.2 follows the same default | Non-zonal (pre-v2 raw resource); single zone | Matches the portal default for Standard SKU and the zone-redundancy story of the Standard Load Balancer in Lab 2.3, at the same price. |
...
| **Subnet ownership** | Lab 2.1 declares every subnet; Lab 2.2 re-declares Auth/World/Database with their NSGs and service endpoints | One template per network, layered through parameters, so a single source of truth owns each subnet | Re-running Lab 2.1 after Lab 2.2 re-declares the spoke subnets without NSGs or service endpoints (Lab 4.4 relies on the latter) - run the labs in order (2.1 → 2.2 → 2.3). Production keeps one template per network so this drift cannot happen. |

...
| **Subnet ownership** | Lab 2.1 declares every subnet; Lab 2.2 re-declares Auth/World/Database with their NSGs and service endpoints | One template per network, layered through parameters, so a single source of truth owns each subnet | Re-running Lab 2.1 after Lab 2.2 re-declares the spoke subnets without NSGs - run the labs in order (2.1 → 2.2 → 2.3). Production keeps one template per network so this drift cannot happen. |

...
| **Service endpoints** | Enabled on the Database subnet for Microsoft.Sql and Microsoft.Storage, and on the World subnet for Microsoft.Storage (Lab 4.4) | No service endpoints; rely on private endpoints (`modSubnetWorld` / `modSubnetDb` in main.bicep) | Service endpoints allow subnet-level access to PaaS without public IP. Cheaper than private endpoints (~€0 vs. ~€7.50/mo) but less isolated. Lab teaches both: endpoints are a good starting point. |
...
| **AVM consumption** | `network-security-group`, `application-security-group`, `virtual-network/subnet` (NSG association + service endpoints) and `bastion-host` (creates its own Standard public IP), called directly from `main.bicep` | Hand-written resources in `security-spoke.bicep` / `security-hub.bicep` (pre-v2) | docs/bicep-standards.md §4. The subnet child module is the AVM way to attach an NSG to an existing subnet; the three subnet loops are chained with an explicit `dependsOn` because concurrent subnet updates on one VNet fail with `AnotherOperationInProgress` (§6.5: genuine sequencing, commented). The module takes service endpoints as plain service names and sets no `locations`, so Azure records them as region-unrestricted (`*`) — equivalent for Lab 4.4's same-region storage account. |
| **Loop over spokes** | A `varSpokes` array (dev, prod) drives every spoke-scoped module (`[for spoke in varSpokes: ...]`) | Two copies of every module; a local "spoke" wrapper module around the AVM calls | One declaration per concern keeps `main.bicep` readable and guarantees dev and prod stay identical; a wrapper module would re-introduce the local-module layer that v2 removes. |

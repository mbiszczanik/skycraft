...
- **Reduction tactics:**
  ...
  - `Deploy-Bicep.ps1` creates the backup policies with Az PowerShell cmdlets (`New-AzRecoveryServicesBackupProtectionPolicy` for the VM policy, `New-AzDataProtectionBackupPolicy` for the blob policy) and checks for an existing policy first, so re-runs do not attempt to create them again.
- **Cleanup reminder:**

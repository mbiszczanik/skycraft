...
- **Cleanup reminder:**
  ...
  - The **NetworkWatcherAgent extension** that `Deploy-Bicep.ps1` installs on the two connection monitor endpoint VMs is not part of any resource group the lab deletes; without explicit cleanup it outlives the lab on every VM that is kept. `Remove-LabResource.ps1` removes it after the connection monitor is gone.
  - Enabling Traffic Analytics makes Azure create its own **`NWTA-<workspace-guid>-<region>` data collection rule and endpoint** next to the Log Analytics Workspace. They are not declared by `main.bicep` and they survive the flow log's deletion. `Remove-LabResource.ps1` deletes the pair once no flow log with Traffic Analytics is left; otherwise they linger until the platform resource group is deleted.
...
- **The Network Watcher itself is re-declared.** The module always PUTs `NetworkWatcher_<location>` in `NetworkWatcherRG` — an idempotent update of the auto-provisioned watcher, which now carries the canonical four tags (`Environment = Production`, the lab's default). `Deploy-Bicep.ps1` still bootstraps the watcher when Azure has not created it and installs the NetworkWatcherAgent extension on both VMs; `Remove-LabResource.ps1` never deletes the watcher — it is Azure-provisioned per region and shared.
- **What cleanup removes beyond the template.** `Remove-LabResource.ps1` runs four steps: connection monitor, flow log, the NetworkWatcherAgent extension on the connection monitor endpoint VMs, and the `NWTA-*` Traffic Analytics data collection rule/endpoint. The last two are side effects of the deployment rather than declared resources, so nothing in the template removes them. The extension step reads the endpoint VMs from the connection monitor *before* deleting it (falling back to the VM names `Deploy-Bicep.ps1` can pick), skips a VM that is already gone, and skips any agent not tagged `Project = SkyCraft` — `Deploy-Bicep.ps1` tags what it installs and leaves a pre-existing agent alone, so an untagged agent belongs to someone else.

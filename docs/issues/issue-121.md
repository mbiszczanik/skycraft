...
Implemented on `fix/lab-3.3-environment-names` as specified, with one deviation: `Deploy-Bicep.ps1`'s subscription deployment name is now `Lab-3.3-Containers-<Environment>` (was `Lab-3.3-Containers`), mirroring Lab 3.4's `deploy-lab3.4-$Environment`, so prod and dev runs keep separate deployment records. Verified offline: 1985/1985 repo-root and 75/75 lab-local Pester cases (65 new in `tests/Lab33-Environment-Names.Tests.ps1`), `tools/Invoke-DryRun.ps1` parse/analyzer/bicep/bicepparams all PASS, markdownlint clean. Verified live, read-only: `Deploy-Bicep.ps1 -Environment prod -WhatIf` resolves `prod-skycraft-swc-rg`, `prodskycraftswcacr01`, `prod-skycraft-swc-aci-auth`, `prod-skycraft-swc-cae-02`, `prod-skycraft-swc-aca-world` and exits 0. At implementation time the subscription held no lab resource group and no container resource at all, so the eight old-scheme orphans and the dev `-aca-world-02` app listed under Risks no longer need removing; the `-AcaName` override remains for any future case.

**Live dev cycle (2026-09-21, MPN subscription, from the branch):** `Deploy-Bicep.ps1 -Environment dev` created `dev-skycraft-swc-rg`, bootstrapped the registry and image, deployed the instance, the environment and `dev-skycraft-swc-aca-world` - exit 0. `Test-Lab.ps1 -Environment dev` - all checks passed, exit 0. `scripts/Invoke-ResourceAudit.ps1` against the live estate - 7 resources in scope, all accounted for. `Remove-LabResource.ps1 -Environment dev -Force` - 27 minutes, exit 0, **but the instance and the registry were still standing**: both deletes had thrown after the long environment delete and every `catch` printed `[INFO] Not found or already deleted.`, while the two `Remove-AzResource` calls ran with `-ErrorAction SilentlyContinue`. Re-issued by hand, both deletes succeeded at once, so the failure was transient (most likely the session after the 24-minute wait) - the cause is unknowable because the script discarded it. That is the failure class #105 fixed for Lab 5.2, so the fix is folded into this branch rather than deferred: each step now resolves its target with `Get-AzResource` first, skips what does not exist, reports a throwing delete as `[ERROR]` with the message, counts it, keeps going, and exits 1 when anything failed. `module-3-compute/3.3-containers/tests/Remove-LabResource.Tests.ps1` (9 cases on `tests/Support/LabScriptStub.psm1`, written first and seen failing) pins the contract. The fixed script re-run against the emptied group reported four `Not found - nothing to delete` and exit 0; the empty `dev-skycraft-swc-rg` was then removed by hand, returning the subscription to the empty state the cycle started from.

...
## **Section 4.1: Configure and Manage Storage Accounts (2 hours)**

**Hours 27-28**: Storage Account Foundation for SkyCraft

- Create and configure storage accounts (Standard GPv2 across 3 environments)
- Configure Azure Storage redundancy (LRS for dev, GRS for production)
- Configure storage account encryption (Microsoft-managed keys, TLS 1.2)
- Manage access keys and understand dual-key rotation
- Apply governance tags (Project, Environment, CostCenter)

**Practical Task**: Deploy 3 storage accounts (platform/dev/prod) with environment-appropriate redundancy, security settings, and governance tags

---

## **Section 4.2: Configure Azure Blob Storage (2 hours)**

**Hours 29-30**: Blob Storage for Game Assets

- Create and configure containers with access levels (Private, Blob, Container)
...
- Configure blob versioning and soft delete (track changes to game files)
- Manage data by using Azure Storage Explorer and AzCopy

**Practical Task**: Create blob containers for SkyCraft game assets, configure lifecycle rules, and upload data using AzCopy

---

## **Section 4.3: Configure Azure Files (1.5 hours)**

**Hours 31-32**: Shared File Storage for Game Servers

- Create and configure file shares in Azure Storage (shared configs for SkyCraft servers)
...
- Troubleshoot SMB connectivity (port 445, CIFS mount options, secure transfer)

**Practical Task**: Create Azure File Shares mounted to SkyCraft VMs for shared configuration, implement snapshot-based disaster recovery

---

## **Section 4.4: Implement Storage Security (2.5 hours)**

**Hours 33-34**: Defense-in-Depth for Storage

- Configure Azure Storage firewalls and virtual networks (restrict access to VNet only)
- Configure Service Endpoints for storage on VNet subnets
- Create and test ad-hoc Shared Access Signature (SAS) tokens
- Configure Stored Access Policies for revocable delegated access
- Configure identity-based access (RBAC) for storage data plane operations

**Practical Task**: Implement defense-in-depth storage security: network isolation, ad-hoc and revocable SAS delegation, and Entra ID RBAC

**Milestone**: Complete storage layer with proper redundancy, security, lifecycle management, and identity-based access integrated with your compute infrastructure

...
- **Az PowerShell vs. `az` CLI**: Prefer **Az PowerShell cmdlets** (`Get-AzContext`, `New-AzSubscriptionDeployment`, ...) as the default in scripts — they return objects, not text. Dropping to the **`az` CLI** is acceptable where its coverage is better (e.g., `az backup`, `az bicep`), but never mix both for the *same* resource within one script, and always follow the `--output json` rule below. The same rule governs the course material that is **operational** rather than educational — lab checklists ([checklist-standards.md](checklist-standards.md) section 3.2) and the prerequisite checks in each `module-*/README.md`: both gate a deployment `Deploy-Bicep.ps1` then makes under the Az context, so an `az` block silently checks a different subscription. `tests/Docs-Az-PowerShell.Tests.ps1` pins both surfaces. Lab guides are **partly** in scope (#141): their teaching steps stay multi-modal — Portal, CLI and PowerShell side by side ([lab-guide-standards.md](lab-guide-standards.md)) — but a guide step that deploys, exports or deletes the lab's own infrastructure is operational like a checklist, and uses the lab's `scripts/` or an Az cmdlet. The same test pins that, as a fixed list of operational `az` verbs rather than a blanket ban, so `az vm resize`, `az storage ...` and the "Option 2: Azure CLI" blocks are untouched. `az bicep` and `az acr build` remain accepted exceptions: the first is a local compile that never reaches a subscription, the second has no cmdlet that builds an image from source.

...
- **Container Registry** (`container-registry/registry`): `networkRuleSetDefaultAction: 'Allow'` — with public access enabled the default `Deny` adds a network rule set that blocks image pulls and imports (Lab 3.3).
- **App Service Plan** (`web/serverfarm`): `zoneRedundant: false` and `skuCapacity: 1` — the module defaults Premium SKUs to zone-redundant with three workers (Lab 3.4).

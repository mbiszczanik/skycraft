...
- Verified against the live subscription. The first run flagged `NetworkWatcher_swedencentral` - a false positive, since Lab 5.3 re-declares and tags it, but its name carries neither the project token nor the dashed prefix; expanding over the location domain fixed it and the re-run is clean. Measured on a set of 26 names afterwards, the audit produced 0 false positives out of 12 real resources and 0 false negatives out of 14 plausible drift names.

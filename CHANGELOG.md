...
- Lab 2.3 `Remove-LabResource.ps1` deleted the private DNS zone while its VNet links were still draining, failing with `Cannot delete resource while nested resources exist` and needing a manual retry; the script now waits for the links to disappear, and exits non-zero with their names if they do not (#97).

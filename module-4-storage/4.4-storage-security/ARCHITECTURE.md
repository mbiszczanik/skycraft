...
- **`-Environment platform` is not usable.** The hub VNet has only `AzureBastionSubnet` and `GatewaySubnet`, so there is no workload subnet to allow through the storage firewall.
